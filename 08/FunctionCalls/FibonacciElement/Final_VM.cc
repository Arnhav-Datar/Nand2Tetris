...
int counter=0;
string name;
//Removes Comments From string given
string rc(string p) { 
    int n = p.length(); 
    string ans; 
    int s=0,m=0; 
...
//Pops whatever value was stored in R13
void pop(){
	of<<"@SP"<<endl;
	of<<"AM=M-1"<<endl;
	of<<"D=M"<<endl;
...
//pushes What was stored in D onto stack
void push(){
	of<<"@SP"<<endl;
	of<<"A=M"<<endl;
	of<<"M=D"<<endl;
...
void memory(string s){
	...
	if(s1.compare("push")==0){
		...
		of<<"@"<<s2<<"_"<<"ret"<<returncount<<name<<endl;
		of<<"D=A"<<endl;
		push();
