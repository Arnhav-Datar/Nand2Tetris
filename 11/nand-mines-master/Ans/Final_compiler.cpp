...
void comSubRoutineCall() {
    ...
    else 
    {
        ...
        else if (fv.count(x1) != 0) ff << "call " << xtrim(fv[x1].F) << "." << x2 << " " << n + 1 << endl;
...
int comExpressionList() {
    ...
    if (isSym(xml[ind], ","))
        goto top;
    while (xml[ind] != "</expressionList>") inc();
    inc();
    return np;
}
void comTerm() {
...
void comLetStat() {
    ...
    if (isSym(xml[ind], "[")) {
        ...
        else if (fv.count(x1) != 0) ff << "pop this " << fv[x1].S << endl;
...
void comSubRoutine() {
    ...
    while (xml[ind] != "<parameterList>") inc();
    inc();
    comParameterList();
